...
TEE_Result ObTeeNullSafeDatumStrCmpEntry(uint32_t param_types, TEE_Param params[7]) {
  // unused parameter
  (void)&param_types;

  ObCollationType cs_type = (ObCollationType)params[0].value.a; // TEE_PARAM_TYPE_VALUE_INPUT
  bool with_end_space = (params[1].value.a != 0) // TEE_PARAM_TYPE_VALUE_INPUT
  bool null_first = (params[2].value.a != 0) // TEE_PARAM_TYPE_VALUE_INPUT
  ObDatum *l = (ObDatum *)params[3].memref.buffer; // TEE_PARAM_TYPE_MEMREF_INPUT
  ObDatum *r = (ObDatum *)params[4].memref.buffer; // TEE_PARAM_TYPE_MEMREF_INPUT
  int cmp_ret = params[5].value.a; // TEE_PARAM_TYPE_VALUE_OUTPUT

  ret = ObTeeNullSafeDatumStrCmp<cs_type, with_end_space, null_first>::cmp(*l, *r, cmp_ret);

  // OB 的具体错误码通过 TEE_Param 参数返回
  params[6].value.a = ret; // TEE_PARAM_TYPE_VALUE_OUTPUT

  // 返回 TEE 的通用错误码，作为调用结果
  return (ret == OB_SUCCESS) ? TEE_SUCCESS : TEE_ERROR_GENERIC;
}
